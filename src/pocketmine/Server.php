...
class Server{
 ...
 private $resourceManager;
	...
	private function titleTick(){
		...
		$usage = round(($u[0] / 4024) / 4024, 2) . "/" . round(($d[0] / 4024) / 4024, 2) . "/" . round(($u[1] / 4024) / 4024, 2) . "/" . round(($u[2] / 4024) / 4024, 2) . " MB @ " . Utils::getThreadCount() . " MeLoNyPE";

		echo "\x1b]0;" . $this->getName() . $this->getFormattedVersion("-") .
			" | ออนไลน์ " . count($this->players) . "/" . $this->getMaxPlayers() .
			" | หน่วยความจำ " . $usage .
			" | U " . round($this->network->getUpload() / 4024, 2) .
			" D " . round($this->network->getDownload() / 4024, 2) .
			" kB/s | TPS " . $this->getTicksPerSecondAverage() .
			" | โหลด " . $this->getTickUsageAverage() . "%\x07";
