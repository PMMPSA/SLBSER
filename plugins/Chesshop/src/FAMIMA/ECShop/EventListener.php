...
class EventListener implements Listener
{
	...
	const ECS = TF::WHITE."[".TF::GREEN."CSHOP".TF::WHITE."]";
		...
		if($lines[0] == "")
		{
			if(is_numeric($lines[1]) and is_numeric($lines[2])  and (preg_match('/^[0-9]+:[0-9]+$/', $lines[3]) or is_numeric($lines[3])) )
			{
				...
				if($this->ecshop->isExistsChests($pos = $b))
				{
					...
					$e->setLine(0, "§l§cร§6้§eา§aน§bค§d้§fา§eข§6อ§bง §c: §f".$p->getName());
					$e->setLine(1, "§l§aร§6า§aค§6า§e §c:§b ".$lines[1]);
					$e->setLine(2, "§d§l⚒§eจ§fำ§eน§fว§eน§d⚒ §c:§6 ".$lines[2]);
					$e->setLine(3, "§l§bขาย §f: §e".$item->getName());
				}else{
