...
class EventListener implements Listener {
    ...
    public function onBreak(BlockBreakEvent $event) {
        ...
        if($island instanceof Island) {
            if(!$event->getPlayer()->isOp() and !in_array(strtolower($event->getPlayer()->getName()), $island->getAllMembers())) {
                $event->getPlayer()->sendPopup(TextFormat::RED . "§c✖§f Bạn phải là một người của hòn đảo này mới có thể phá hủy block trên nó!");
                $event->setCancelled();
            }
            else  {
                if($event->getBlock()->getId() == Block::COBBLESTONE) {
                    $items = [];
                    $items[] = Item::get(264);
                    $this->addItemMultipleTimes(3, Item::get(388), $items);
                    $this->addItemMultipleTimes(5, Item::get(265), $items);
                    $this->addItemMultipleTimes(20, Item::get(266), $items);
                    $this->addItemMultipleTimes(30, Item::get(Item::COAL), $items);
					$this->addItemMultipleTimes(40, Item::get(Item::SAND), $items);
    ...
    public function onPlace(BlockPlaceEvent $event) {
        ...
        if($island instanceof Island) {
            if(!$event->getPlayer()->isOp() and !in_array(strtolower($event->getPlayer()->getName()), $island->getAllMembers())) {
                $event->getPlayer()->sendPopup(TextFormat::RED . "§c✖§f Bạn phải là một người của hòn đảo này mới có thể đặt block trên nó!");
                $event->setCancelled();
            }
        }
    }
