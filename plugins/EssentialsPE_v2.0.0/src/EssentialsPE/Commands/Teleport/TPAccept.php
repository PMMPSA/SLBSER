...
class TPAccept extends BaseCommand{
    ...
    public function execute(CommandSender $sender, $alias, array $args): bool{
        ...
        if(!($request = $this->getAPI()->hasARequest($sender))){
            $sender->sendMessage(TextFormat::RED . "§e§lยังไม่มีไคร§ctpa§eหาคุณ");
            return false;
        }
        switch(count($args)){
            case 0:
            ...
            case 1:
                if(!($player = $this->getAPI()->getPlayer($args[0]))){
                    $sender->sendMessage(TextFormat::RED . "§a• §e§lไม่พบผู้เล่นดังนี้ §a•");
                    return false;
                }
        ...
        $sender->sendMessage(TextFormat::GREEN . "§l§bกำลังวาปมาหา§f...");
